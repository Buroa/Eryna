...
/**
 * Holds the region music player.
 * @author Steve
 */
public final class RegionMusic {

	/**
	 * Holds the song values.
	 */
	private static final Music[] songs = { new Music(76, 3200, 3199, 3273, 3302), // Harmony
	...
	private static Music getMusicId(Player player) {
		final int x = player.getPosition().getX(), y = player.getPosition().getY();
		for (final Music song : songs)
			if (x >= song.getSwX() && x <= song.getNeX() && y >= song.getSwY() && y <= song.getNeY())
				return song;
		return null;
	}

	/**
	 * Plays the music for the player.
	 * @param player The player.
	 */
	public static void playMusic(Player player) {
		final Music song = getMusicId(player);
		if (song == null)
			player.getSettings().playSong(2);
		else
			player.getSettings().playSong(song.getMusic());
	}
}