...
public final class CreditsCommandListener implements CommandListener {
	 ...
	 * Apollo took several people thousands of
	 * hours to create. We released it to the world for free and it isn't much
	 * to ask to leave this command in. It isn't
	 * very obtrusive and gives us some well-deserved recognition for the work
	 * we have done. Thank you! The list of
	 * authors is generated from the plugin manager. If you create a custom
	 * plugin, make sure you add your name to the
	...
	public void execute(Player player, Command command) {
		final PluginManager mgr = World.getWorld().getPluginManager();
		final Iterator<String> it = mgr.createAuthorsIterator();
		int pos = 0;
		...
		for (; pos < QuestConstants.QUEST_TEXT.length; pos++) {
			final String text = it.hasNext() ? it.next() : "";
