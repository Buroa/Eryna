...
public final class EquipmentUpdater {
	...
	private static int getWeaponType(ItemDefinition def) {
		...
		if (name == null)
			name = "null";
		for (final String element : EquipmentConstants.CAPES)
			if (name.contains(element))
				return 1;
		for (final String element : EquipmentConstants.HATS)
			if (name.contains(element))
				return 0;
		for (final String element : EquipmentConstants.BOOTS)
			if (name.endsWith(element) || name.startsWith(element))
				return 10;
		for (final String element : EquipmentConstants.GLOVES)
			if (name.endsWith(element) || name.startsWith(element))
				return 9;
		for (final String element : EquipmentConstants.SHIELDS)
			if (name.contains(element))
				return 5;
		for (final String element : EquipmentConstants.AMULETS)
			if (name.endsWith(element) || name.startsWith(element))
				return 2;
		for (final String element : EquipmentConstants.ARROWS)
			if (name.endsWith(element) || name.startsWith(element))
				return 13;
		for (final String element : EquipmentConstants.RINGS)
			if (name.endsWith(element) || name.startsWith(element))
				return 12;
		for (final String element : EquipmentConstants.BODY)
			if (name.contains(element))
				return 4;
		for (final String element : EquipmentConstants.LEGS)
			if (name.contains(element))
				return 7;
		for (final String element : EquipmentConstants.WEAPONS)
			if (name.endsWith(element) || name.startsWith(element))
				return 3;
		return -1;
	}
	...
	private static boolean isFullBody(ItemDefinition def) {
		...
		if (name == null)
			name = "null";
		for (final String element : EquipmentConstants.FULL_BODIES)
			if (name.contains(element))
				return true;
		return false;
	}
	...
	private static boolean isFullHat(ItemDefinition def) {
		...
		if (name == null)
			name = "null";
		for (final String element : EquipmentConstants.FULL_HATS)
	...
	private static boolean isFullMask(ItemDefinition def) {
		...
		if (name == null)
			name = "null";
		for (final String element : EquipmentConstants.FULL_MASKS)
			if (name.endsWith(element))
				return true;
		return false;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		final DataOutputStream os = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(
				"data/equipment-" + release + ".dat")));
		try {
			...
			try {
				...
				for (int id = 0; id < defs.length; id++) {
					...
					final int type = getWeaponType(def);
					os.writeByte(type);
					if (type != -1) {
						...
						// Runewiki fucked their articles again, cant scrape...
						final double[] bonuses = new double[18];
