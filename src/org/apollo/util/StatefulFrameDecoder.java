...
/**
 * A stateful implementation of a {@link FrameDecoder} which may be extended and
 * used by other classes. The current
 * state is tracked by this class and is a user-specified enumeration. The state
 * may be changed by calling the {@link StatefulFrameDecoder#setState(Enum)}
 * method. The current state is supplied as a parameter in the
 ...
 * methods. This class is
 * not thread safe: it is recommended that the state is only set in the decode
 * methods overriden. {@code null} states
 * are not permitted. This means you cannot use {@link VoidEnum} like used in a
 * {@link ReplayingDecoder}. If you do not
 * need state management, the {@link FrameDecoder} class should be used instead.
 * @author Graham
 */
...
public abstract class StatefulFrameDecoder<T extends Enum<T>> extends FrameDecoder {
	 ...
	 * (non-Javadoc)
	 * @see
	 * org.jboss.netty.handler.codec.frame.FrameDecoder#decodeLast(org.jboss
	 ...
	 * override this method, but it is not
