...
public final class BinaryPlayerLoader implements PlayerLoader {
	 ...
	 */
	private static final Position SPAWN_POSITION = new Position(3094, 3494);
	...
	public PlayerLoaderResponse loadPlayer(PlayerCredentials credentials) throws Exception {
		...
		try {
			...
			if (banned)
				return new PlayerLoaderResponse(LoginConstants.STATUS_ACCOUNT_DISABLED, p);
			...
			for (int i = 0; i < size; i++) {
				// TODO read long is always downcased, does that matter?
