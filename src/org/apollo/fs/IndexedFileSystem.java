 ...
 * of a data file and index files. Index
...
public final class IndexedFileSystem implements Closeable {
	...
	public void close() throws IOException {
		...
		for (final RandomAccessFile index : indices)
			if (index != null)
				synchronized (index) {
					index.close();
				}
	...
	public ByteBuffer getCrcTable() throws IOException {
		if (readOnly) {
			...
			final int archives = getFileCount(0);
			// the hash
			int hash = 1234;
			// the CRCs
			final int[] crcs = new int[archives];
			// calculate the CRCs
			final CRC32 crc32 = new CRC32();
			for (int i = 1; i < crcs.length; i++) {
				crc32.reset();
				final ByteBuffer bb = getFile(0, i);
			...
			final ByteBuffer buf = ByteBuffer.allocate(crcs.length * 4 + 4);
	...
	public ByteBuffer getFile(FileDescriptor fd) throws IOException {
		...
		for (int i = 0; i < blocks; i++) {
			// read header
			final byte[] header = new byte[FileSystemConstants.HEADER_SIZE];
			synchronized (data) {
				data.seek(ptr);
	...
	private int getFileCount(int type) throws IOException {
		...
		final RandomAccessFile indexFile = indices[type];
